...
class VGG16RoIHead(nn.Module):
    ...
    ###
    # @brief RoIPooling network
    # @param x model의 Feature map
    # @param roi sample rois batch 적용 시 배치 크기만큼의 anchor 선택 (현재는 batch를 사용하지 않으므로 0)
    # @param roi_indices roi (현재는 batch를 사용하지 않으므로 0)
    # @return roi_cls_locs : result of pooling classification
    # @return roi_scores : result of pooling score
    ###
    def forward(self, x, rois, roi_indices):
        ...
        indices_and_rois = t.cat([roi_indices[:, None], rois], dim=1) # 128 * (1(배경) + 4)
